...
typedef struct _mi_imported_t {
    struct _mi_imported_t *next;
    int         stat;   // 0 - ok, 1 - not changed
    char        *short_fpr;
    char        *info;
    char        fpr[1];
} mi_imported_t;

typedef struct _mictx_t {
    IOBUF           b;
    char            *pass;
    mi_imported_t   *imported;
    int             rc;
    char            *filename;
    int             dec;
} MICTX;

mi_imported_t *MI_IMPORT;

//#define PRINT_FUNC printf("++ [%p]  %s  -   %s ++\n", pthread_self(), __FILE__, __FUNCTION__);
#define PRINT_FUNC
