...
FNAMEX(zfree)( void *a FNAMEPRT )
{
    ...
    m_check(p);
    if( m_is_secure(a) ) {
        /*printf("--- MSFREE 0x%lx - 0x%lx\n", (long)a, (long)(p-EXTRA_ALIGN-4));*/
        secmem_free(p-EXTRA_ALIGN-4);
    } else {
        used_memory -= m_size(a);
        /*printf("--- MFREE 0x%lx - 0x%lx\n", (long)a, (long)(p-EXTRA_ALIGN-4));*/
        free(p-EXTRA_ALIGN-4);
    }
#else
    if( m_is_secure(a) ) {
        /*printf("--- SFREE 0x%lx - 0x%lx\n", (long)a, (long)(p-EXTRA_ALIGN-4));*/
        secmem_free(p);
    } else {
