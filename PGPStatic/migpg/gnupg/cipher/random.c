...
pthread_mutex_t _rc_lock_rnd = PTHREAD_RECURSIVE_MUTEX_INITIALIZER;
static void
_lock(int c) {
    if (pthread_mutex_lock(&_rc_lock_rnd)) {
        log_fatal("-------- ERROR OF LOCK [%u] %s-----\n", 
                c, __FILE__);
        exit(1);
    }
    /*printf("--- LOCK   (%s) %u ---\n", __FILE__, c);*/
...
_unlock(int c) {
    /*printf("--- UNLOCK (%s) %u ---\n", __FILE__, c);*/
    if (pthread_mutex_unlock(&_rc_lock_rnd)) {
        log_fatal("-------- ERROR OF UNLOCK [%u] %s-----\n", 
                c, __FILE__);
        exit(1);
    }
}
