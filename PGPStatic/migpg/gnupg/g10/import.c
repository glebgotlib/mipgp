...
void
mi_store_import(PKT_public_key *pk, PKT_secret_key *sk, 
        char *userid, int reason) {

    byte array[MAX_FINGERPRINT_LEN], *s;
    char buf[MAX_FINGERPRINT_LEN*2+30], *p;
    size_t i, n;
    mi_imported_t *mii = NULL;
    ...
    if (sk) {
        n = snprintf(NULL, 0, "%s%csec %4u%c/%s %s %s%c%s",
                buf, '\0',
                nbits_from_sk( sk ),
                pubkey_letter( sk->pubkey_algo ),
                keystr_from_sk(sk), datestr_from_sk(sk),
                userid, '\0', keystr_from_sk(sk));
        mii = xmalloc(sizeof(mi_imported_t) + n + 1);
        n = snprintf(mii->fpr, n + 1, "%s%csec %4u%c/%s %s %s%c%s",
                buf, '\0',
                nbits_from_sk( sk ),
                pubkey_letter( sk->pubkey_algo ),
                keystr_from_sk(sk), datestr_from_sk(sk),
    ...
    if (pk) {
        n = snprintf(NULL, 0, "%s%cpub %4u%c/%s %s %s%c%s",
                buf, '\0',
                nbits_from_pk( pk ),
                pubkey_letter( pk->pubkey_algo ),
                keystr_from_pk(pk), datestr_from_pk(pk),
                userid, '\0', keystr_from_pk(pk));
        mii = xmalloc(sizeof(mi_imported_t) + n + 1);
        n = snprintf(mii->fpr, n + 1, "%s%cpub %4u%c/%s %s %s%c%s",
                buf, '\0',
                nbits_from_pk( pk ),
                pubkey_letter( pk->pubkey_algo ),
                keystr_from_pk(pk), datestr_from_pk(pk),
    ...
    if (mii) {
        mii->info = mii->fpr + strlen(mii->fpr) + 1;
        mii->short_fpr = mii->info + strlen(mii->info) + 1;
	    ...
	    int from_sk, import_filter_t filter, void *filter_arg)
{
    ...
    mi_store_import(pk, NULL, uidnode->pkt->pkt.user_id->name, 0);
        /*mi_imported_t *mii = xmalloc(sizeof(mi_imported_t));
        snprintf(mii->fpr, 20, "%s", keystr_from_pk(pk));
        snprintf(mii->info, 1024, "pub  %4u%c/%s %s %s",
                nbits_from_pk( pk ),
                pubkey_letter( pk->pubkey_algo ),
                keystr_from_pk(pk), datestr_from_pk(pk),
	...
	if( n_uids || n_sigs || n_subk || n_sigs_cleaned || n_uids_cleaned) {
        ...
        mi_store_import(pk, NULL, uidnode->pkt->pkt.user_id->name, 2);
        /*mi_imported_t *mii = xmalloc(sizeof(mi_imported_t));
        snprintf(mii->fpr, 20, "%s", keystr_from_pk(pk));
        snprintf(mii->info, 1024, "pub  %4u%c/%s %s %s",
                nbits_from_pk( pk ),
	...
	else
	  {
        ...
        mi_store_import(pk, NULL, uidnode->pkt->pkt.user_id->name, 1);
        /*mi_imported_t *mii = xmalloc(sizeof(mi_imported_t));
        snprintf(mii->fpr, 20, "%s", keystr_from_pk(pk));
        snprintf(mii->info, 1024, "pub  %4u%c/%s %s %s",
                nbits_from_pk( pk ),
                   ...
                   import_filter_t filter, void *filter_arg)
{
	...
	stats->secret_imported++;
	  ...
	  print_import_ok (NULL, sk, 1|16);
        ...
        mi_store_import(NULL, sk, uidnode->pkt->pkt.user_id->name, 0);
        /*mi_imported_t *mii = xmalloc(sizeof(mi_imported_t));
        snprintf(mii->fpr, 20, "%s", keystr_from_sk(sk));
        snprintf(mii->info, 1024, "sec  %4u%c/%s %s %s",
                nbits_from_sk( sk ),
                pubkey_letter( sk->pubkey_algo ),
                keystr_from_sk(sk), datestr_from_sk(sk),
                uidnode->pkt->pkt.user_id->name);
    ...
    { /* we can't merge secret keys */
        ...
        mi_store_import(NULL, sk, uidnode->pkt->pkt.user_id->name, 1);
        /*mi_imported_t *mii = xmalloc(sizeof(mi_imported_t));
        snprintf(mii->fpr, 20, "%s", keystr_from_sk(sk));
        snprintf(mii->info, 1024, "sec  %4u%c/%s %s %s",
                nbits_from_sk( sk ),
